    ...
    <aside style={{
      ...
      <div style={{ padding: "16px 14px 8px" }}>
        <button onClick={onNewTicket} style={{
          width: "100%", padding: "10px 0", background: "var(--accent)", color: "white",
      ...
      <nav style={{ padding: "4px 14px" }}>
        {[
          ...
          onMouseLeave={e => { if (activeScreen !== item.id) e.currentTarget.style.background = "transparent"; }}>
            <Icon name={item.icon} size={15} color={activeScreen === item.id ? "white" : "var(--sidebar-text)"} />
            {item.label}
          </button>
        ))}

        {activeScreen === "processing" && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 7, background: "var(--sidebar-active)", marginBottom: 2 }}>
            <Icon name="zap" size={15} color="white" />
            <span style={{ fontSize: 13, fontWeight: 600, color: "white" }}>Agent Pipeline</span>
            <div style={{ marginLeft: "auto", width: 6, height: 6, borderRadius: "50%", background: "var(--accent-light)", animation: "pulse-ring 1.4s ease-out infinite" }} />
          </div>
        )}
      ...
      <div style={{ padding: "0 14px 6px" }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: "var(--sidebar-muted)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 6, padding: "0 10px" }}>
          Recent Tickets
        </div>
        <div style={{ overflowY: "auto", maxHeight: 340 }}>
          {tickets.map((t) => (
            <div key={t.id} style={{ padding: "8px 10px", borderRadius: 7, marginBottom: 2, cursor: "pointer", transition: "background 0.15s" }}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 2 }}>
                ...
                <span style={{ marginLeft: "auto" }}>
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: t.status === "resolved" ? "var(--success)" : "var(--warning)" }} />
                </span>
              </div>
              <div style={{ fontSize: 12, color: "var(--sidebar-text)", fontWeight: 500, lineHeight: 1.3 }}>{t.subject}</div>
      ...
      <div style={{ marginTop: "auto", padding: "14px 14px", borderTop: "1px solid oklch(0.22 0.02 255)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          <div style={{ width: 30, height: 30, borderRadius: "50%", background: "oklch(0.40 0.12 250)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "white" }}>A</div>
          <div>
